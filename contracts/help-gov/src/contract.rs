...
use help_protocol::common::OrderBy;
use help_protocol::gov::{
...
pub fn instantiate(
    ...
    let config = Config {
        help_token: deps.api.addr_canonicalize(&msg.help_token)?,
...
pub fn receive_cw20(
    ...
    if config.help_token != deps.api.addr_canonicalize(info.sender.as_str())? {
...
pub fn end_poll(deps: DepsMut, env: Env, poll_id: u64) -> StdResult<Response> {
    ...
    let (quorum, staked_weight) = if state.total_share.u128() == 0 {
        ...
        let staked_weight = load_token_balance(
            &deps.querier,
            deps.api.addr_humanize(&config.help_token)?.to_string(),
            &state.contract_addr,
        )?
    ...
    if tallied_weight == 0 || quorum < config.quorum {
        ...
        if !a_poll.deposit_amount.is_zero() {
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.help_token)?.to_string(),
                funds: vec![],
...
fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        help_token: deps.api.addr_humanize(&config.help_token)?.to_string(),
